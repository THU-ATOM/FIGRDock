...
class RemoveHydrogenPocketDataset(BaseWrapperDataset):
    def __init__(
        ...
        self.add_1dfeature_list = add_1dfeature_list
        self.add_2dfeature_list = add_2dfeature_list
        self.set_epoch(None)
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        if self.remove_hydrogen:
            ...
            if len(self.add_1dfeature_list) != 0:
                for feat in self.add_1dfeature_list:
                    dd[feat] = dd[feat][mask_hydrogen]
                for feat in self.add_2dfeature_list:
                    dd[feat] = dd[feat][mask_hydrogen, :][:, mask_hydrogen]
        if not self.remove_hydrogen and self.remove_polar_hydrogen:
            ...
            if end_idx != 0:
                ...
                if len(self.add_1dfeature_list) != 0:
                    for feat in self.add_1dfeature_list:
                        dd[feat] = dd[feat][:-end_idx]
                    for feat in self.add_2dfeature_list:
                        dd[feat] = dd[feat][:-end_idx][:-end_idx]
        dd[self.atoms] = atoms
