...
from predictor.processor import read_molecule

warnings.filterwarnings(action="ignore")

# Utils - adapted from public Unimol: unimol.utils.conf_gen_cal_metrics

def single_conf_gen(tgt_mol, num_confs=1000, seed=0, lig_name=None):
    ...
    mol_coords = mol.GetConformer().GetPositions().astype(np.float32)
    noh_mask = [atom.GetSymbol()!="H" for atom in mol.GetAtoms()]
    mol_center = mol_coords[noh_mask].mean(axis=0)
    mol = Chem.AddHs(mol)
    ps = AllChem.ETKDGv3()
    ps.randomSeed = seed
    ps.numThreads = 0
    ...
    mol_coords = mol.GetConformer().GetPositions().astype(np.float32)
    noh_mask = [atom.GetSymbol()!="H" for atom in mol.GetAtoms()]
    new_mol_center = mol_coords[noh_mask].mean(axis=0)
    mol = add_coord(mol, mol_center-new_mol_center)
...
def opt_6t(
    ...
    * verbose: bool. Verbose optimization.
    ...
    noh_mask = th.tensor([a.GetAtomicNum() != 1 for a in this_mol.GetAtoms()])
    ...
    def mse(x: th.Tensor, y: th.Tensor, mask: th.Tensor=None) -> th.Tensor:
        if mask is not None:
            return x, (x[mask] - y[mask]).square().sum(dim=-1).mean(dim=-1)
    ...
    for o in range(opt_steps):
        def closure():
            ...
            if other_coords is not None:
                coords, loss = mse(coords, other_coords, noh_mask)
            elif custom_func is not None:
...
def single_refine(
    ...
    if superimpose:
        ...
        # (N, 3) -> (N, 3)
        conf_mol_init = copy.deepcopy(conf_mol)
        lig_name=input_ligand_path.split("/")[-1].split(".")[0]
        for i in range(3*num_6t_trials):
            conf_mol = single_conf_gen(conf_mol_init, num_confs=1, seed=i, lig_name=lig_name)
            # (N, 3)
        ...
        Chem.MolToMolFile(conf_mol, output_ligand_path, kekulize=True)
        in_lig = conf_mol
    ...
    if min_clash:
        ...
        Chem.MolToMolFile(new_mol, output_ligand_path, kekulize=True)

    if label_ligand_path is not None:
        # label_lig = Chem.MolFromMolFile(label_ligand_path, sanitize=True, removeHs=True)
        # Chem.SanitizeMol(label_lig, sanitizeOps=Chem.SanitizeFlags.SANITIZE_ALL, catchErrors=True)
        # label_lig = Chem.RemoveHs(label_lig)
        label_lig = read_molecule(label_ligand_path, sanitize=True, remove_hs=True)
        label_coords = get_coord(label_lig)
        label_no_h_mask = [a.GetSymbol() != 'H' for a in label_lig.GetAtoms()]
        label_coords = label_coords[label_no_h_mask]
