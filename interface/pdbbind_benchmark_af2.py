...
parser.add_argument(
    ...
    help='data path for generating test set'
)
parser.add_argument(
    "--data_csv",
    type=str,
...
parser.add_argument(
    "--output-sdf-path",
    type=str,
    default="/data/protein/BC_Data/Docking_Data/infer_pdbbind_af2",
...
if args.test_lmdb.split("/")[-1] == "test_apo.lmdb":
    is_apo = True
elif args.test_lmdb.split("/")[-1] == "test_holo.lmdb":
    is_apo = False
else:
...
for idx in range(len(_keys)):
    ...
    complex_name = data["pocket"].split("/")[-1].split("_")[1] if is_apo else data["pocket"].split("/")[-1].split("_")[0]
    data_info = df[df['pdb'] == complex_name]
    ligand_file = os.path.join(args.data_path, data_info['ligandFile'].item()[1:])
    mol = read_molecule(ligand_file)
    if mol == None: 
        print("bad case for: ligand", complex_name)
        continue
    output_ligand_name.append(complex_name)
    if is_apo:
        input_protein.append(os.path.join(args.data_path, data_info['af2File'].item()[1:]))
    else:
        input_protein.append(os.path.join(args.data_path, data_info['pdbFile'].item()[1:]))
...
if is_apo: # if apo
    ...
    input_protein, target_protein, predict_pocket_atom_dict =  get_predict_pdb(lmdb_file, pkl_file, args.batch_size, args.conf_size, predict_sdf_dir, args.max_pocket_atoms, pocket_dict, args.data_path, df)
