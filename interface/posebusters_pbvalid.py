...
parser.add_argument(
    "--out-dir",
    type=str,
    default='/data/protein/BC_Data/Docking_Data/infer/log_posebusters/csvs',
)
parser.add_argument(
    "--meta-csv",
    type=str,
...
for i in tqdm.tqdm(range(len(df))):
    ...
    ligand = ligand_protein["lig_code"]
    protein = ligand_protein["pdb_code"]
    holo_ligand_path = os.path.join(args.holo_ligand_path, f"{protein}_{ligand}.sdf")
    protein_path = os.path.join(args.protein_path, f"{protein}.pdb")
    apo_ligand_path = os.path.join(args.apo_ligand_path, f"{protein}_{ligand}.sdf")
    assert os.path.exists(holo_ligand_path)
    assert os.path.exists(protein_path)
    if not os.path.exists(apo_ligand_path): 
        print("apo not exist for: ", apo_ligand_path)
        continue
