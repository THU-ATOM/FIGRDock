...
parser.add_argument(
    "--data_path",
    type=str,
    default="/data/protein/BC_Data/Docking_Data/unimol_posebusters_eval_sets/posebusters/",
...
parser.add_argument(
    "--test-lmdb",
    type=str,
    default="/data/protein/BC_Data/Docking_Data/posebusters_test/bindnet_8A_all_with_Atom/test_holo.lmdb",
    help='data path for test lmdb'
)
parser.add_argument(
    "--output-sdf-path",
    type=str,
    default="/data/protein/BC_Data/Docking_Data/infer_posebusters",
...
meta_df = pd.read_csv(args.meta_info_file)
for idx in range(len(_keys)):
    datapoint_pickled = env.begin().get(f"{idx}".encode("ascii"))
    data = pickle.loads(datapoint_pickled)
    pdb_code = data["pocket"].split("/")[-1].split(".")[0]
    lig_code = meta_df[meta_df["pdb_code"]==pdb_code]["lig_code"].values[0]
    output_ligand_name.append(f"{pdb_code}_{lig_code}")
    if use_apo:
        protein_file = os.path.join(args.data_path, "protenix_proteins", f"{pdb_code}.pdb")
    else:
        protein_file = os.path.join(args.data_path, "proteins", f"{pdb_code}.pdb")
    ligand_file = os.path.join(args.data_path, "ligands", f"{pdb_code}_{lig_code}.sdf") 
    input_ligand.append(ligand_file)
    input_protein.append(protein_file)
...
if use_apo: # if apo
    ...
    input_protein, target_protein, predict_pocket_atom_dict =  get_predict_pdb(lmdb_file, pkl_file, args.batch_size, args.conf_size, predict_sdf_dir, args.max_pocket_atoms, pocket_dict) # , args.data_path
