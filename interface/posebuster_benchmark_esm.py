...
parser.add_argument(
    "--test-lmdb",
    type=str,
    default="/data/protein/BC_Data/Docking_Data/posebusters_test/bindnet_8A_all_with_Atom_v2/test_apo.lmdb",
...
for idx in tqdm(range(len(_keys))):
    ...
    if use_apo:
        protein_file = os.path.join(args.data_path, f"mix_apo_proteins", f"{pdb_code}.pdb")
    else:
        protein_file = os.path.join(args.data_path, f"{pdb_code}_{lig_code}", f"{pdb_code}_{lig_code}_protein_fixed.pdb")
    ligand_file = os.path.join(args.data_path, f"{pdb_code}_{lig_code}", f"{pdb_code}_{lig_code}_ligand.sdf")
